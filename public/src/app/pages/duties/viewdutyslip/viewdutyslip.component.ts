...
export class ViewdutyslipComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.auth.user.subscribe(data=>{
      ...
      this.uploadService.getFile(this.user.companyName,"closeDutyPics/Pictures/"+this.duty.id,"mapRoute.jpg").subscribe(data=>{
      ...
      // })
      this.uploadService.getFile(this.user.companyName,"closeDutyPics/Pictures/"+this.duty.id,"signature.jpg").subscribe(data=>{
    ...
    this.dutiesService.getEndedCloseDutyAdditionalCharges(this.duty).subscribe(data => {
      ...
      this.additionalCharges.forEach(element => {
        ...
        if(element.fileName != 'undefined' && element.fileName != null) {
          this.uploadService.getPresignedUrl(this.user.companyName,element.fileName,'closeDutyPics/Pictures/'+this.data.id).subscribe(data => {
    ...
    this.dutiesService.getDutySlipSummary(this.duty).subscribe(data => {
      console.log(data)
      this.dutySummary = data[0];

      this.uploadService.getPresignedUrl(this.user.companyName,this.dutySummary.dutySlipFileName,'dutySlip').subscribe(data => {
  ...
  getRouteInfo(){
    this.uploadService.getPresignedUrl(this.user.companyName,'route_info.csv','closeDutyPics/RouteCSV/'+this.duty.id).subscribe(data => {
      if(data.length > 0) {
