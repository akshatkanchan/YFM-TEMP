...
import { CustomerGroup, NewcustomergroupComponent } from '../../masters/newcustomergroup/newcustomergroup.component';
...
import { InvoiceService } from '../../masters/createinvoice/invoice.service';
...
@Component({
  ...
  templateUrl: './addcustomerinbooking.component.html',
  styleUrls: ['./addcustomerinbooking.component.scss']
})
export class AddcustomerinbookingComponent implements OnInit {
    ...
    else
    {
      ...
      this.customerService.addCustomer(this.customer).subscribe(data=>{
        if(data.affectedRows==1){
          ...
          if(this.dutyTypesFormCtrl.length > 0) {
            this.dutyTypesFormCtrl.value.forEach(element => {
              element.ownerId = this.user.ownerId;
              element.customerId = data.insertId;
              this.customerService.addCustomerLimitDutyType(element).subscribe();
              console.log(element)
            });
          }
        }
        ...
        this.matDialogRef.close(this.param.data);
      });
     // this.customer.cid='';
    //  this.customer.name='';
     // this.customer.date='';
    ...
    // }
      ...
      this.customerService.updateCustomer(this.customer).subscribe(data=>{
        ...
        if(this.dutyTypesFormCtrl.length > 0) {
          this.dutyTypesFormCtrl.value.forEach(element => {
            if(element.id) {
              this.customerService.updateCustomerLimitDutyType(element).subscribe();          
            }
            else {
              element.ownerId = this.user.ownerId;
              element.customerId = this.customer.id;
        ...
        this.deletedDutyType.forEach(element => {
          this.customerService.deleteCustomerLimitDutyType(element).subscribe();
        })
      },err=>{},()=>{
