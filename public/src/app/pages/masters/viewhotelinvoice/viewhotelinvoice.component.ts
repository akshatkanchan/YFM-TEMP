...
@Component({
  ...
  templateUrl: './viewhotelinvoice.component.html',
  styleUrls: ['./viewhotelinvoice.component.scss']
})
export class ViewhotelinvoiceComponent implements OnInit {
  ...
  downloadInvoice() {
    ...
    this.uploadService.downloadPDF(pdfData,this.invoiceDetails.id+"_Invoice_"+moment().format("DD-MM-YYYY"),"hotelInvoice",this.user,this.companyProfile,this.bankAccount).subscribe(data=>saveAs(data,this.invoiceDetails.id+"_Invoice_"+moment().format("DD-MM-YYYY")),err=>console.log(err),()=>{      
