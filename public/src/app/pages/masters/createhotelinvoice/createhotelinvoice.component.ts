...
@Component({
  ...
  templateUrl: './createhotelinvoice.component.html',
  styleUrls: ['./createhotelinvoice.component.scss']
})
export class CreatehotelinvoiceComponent implements OnInit {
      ...
      this.invoiceService.getInvoices(temp).subscribe(data=>{
        ...
        this.companyProfileService.getCompanyProfile(temp).subscribe(data => {
          if(data[0].hotelInvoicePrefix == null) {
            ...
            this.prefix = data[0].hotelInvoicePrefix;
          }
  ...
  getBookings(){
    this.hotelService.getHotelBookingsForCustomer(this.bookingData).subscribe(data=>{
      console.log(data)
      this.selectedHotelBookings=data
    })
  }
  viewHotels() {
    if(this.selectedCustomer.id!=null) {
      ...
      console.log(temp);
      var dialTemp = this.dialog.open(HotelbookingsinvoiceComponent, {autoFocus:false,disableClose:true, data: {temp}});
  ...
  calculateTotal()
  {
    ...
    this.selectedHotelBookings.forEach(element=>{
      this.totalTaxablePrice=Number(this.totalTaxablePrice)+Number(element.intTotal)
  ...
  addInvoice()
  {
    ...
    console.log(temp)
    this.invoiceService.addInvoiceHotel(temp).subscribe(data=>{},err=>{},()=>{
