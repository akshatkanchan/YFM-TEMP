...
@Component({
  ...
  templateUrl: './viewflightinvoice.component.html',
  styleUrls: ['./viewflightinvoice.component.scss']
})
export class ViewflightinvoiceComponent implements OnInit {
  ...
  downloadInvoice() {
    ...
    this.uploadService.downloadPDF(pdfData,this.invoiceDetails.id+"_Invoice_"+moment().format("DD-MM-YYYY"),"flightInvoice",this.user,this.companyProfile,this.bankAccount).subscribe(data=>saveAs(data,this.invoiceDetails.id+"_Invoice_"+moment().format("DD-MM-YYYY")),err=>console.log(err),()=>{
