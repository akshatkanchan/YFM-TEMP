...
@Component({
  ...
  templateUrl: './mailhotelinvoice.component.html',
  styleUrls: ['./mailhotelinvoice.component.scss']
})
export class MailhotelinvoiceComponent implements OnInit {
  ...
  ngOnInit() {
    this.auth.user.subscribe(data=>{
      ...
      this.invoiceService.getHotelsInInvoice(temp).subscribe(data => {
        console.log(data)  
        ...
        data.forEach(element => {
          this.bookings.push({
            ...
            checkInDate: moment(element.startDate).format("YYYY-mm-DD"),
            checkOutDate: moment(element.startDate).format("YYYY-mm-DD")
          })          
  ...
  async printInvoice() {
    ...
    await this.uploadService.generateInvoicePDF(pdfData,this.invoiceId+"_Invoice_"+moment().format("DD-MM-YYYY"),"hotelInvoice",this.user,this.companyProfile,this.bankAccount)
...
export interface HotelBookings {
  id: string,    
  bookedBy: string,
  checkInDate: string,
  checkOutDate: string
